...
# GridSearchCV로 RF 하이퍼파라미터 튜닝 + 5-fold 교차검증
import os
import json
import numpy as np
import joblib
...
def main():
    ...
    # 내부 CV (튜닝용). 샘플/클래스 수에 맞게 n_splits는 필요시 조정 가능.
    ...
    grid = GridSearchCV(
        ...
        cv=inner_cv,
        scoring="f1_macro",  # 클래스 많아질 때 macro F1 기준이 더 적절
        n_jobs=-1,
        verbose=1,
    )
    ...
    # -----------------------------
    # 2) 최적 파라미터로 5-Fold 교차검증
    # -----------------------------
    print("\n=== 5-Fold Cross Validation with Best Params ===")

    def create_rf():
        # GridSearch에서 찾은 최적 파라미터 + 공통 옵션
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X_flat, y), 1):
        print(f"\n{'=' * 60}")
    ...
    # 전체 교차검증 결과 요약
    print(f"\n{'=' * 60}")
    print("=== Cross Validation Summary (with Best Params) ===")
    print(f"{'=' * 60}")
    ...
    # 전체 데이터에 대한 예측 (참고용)
    y_pred_full = final_clf.predict(X_flat)
    acc_full = accuracy_score(y, y_pred_full)

    print(f"\n{'=' * 60}")
    print("=== Full Dataset Results (참고용) ===")
    print(f"{'=' * 60}")
    print(f"Full dataset accuracy (training): {acc_full * 100:.2f}%")

    print("\nClassification Report on full dataset:")
    print(
        classification_report(
            y, y_pred_full, target_names=target_names, zero_division=0
        )
    )
