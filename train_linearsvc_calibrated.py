...
def main():
    ...
    print("\n=== 5-Fold CV (Calibrated LinearSVC) ===")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(cv_outer.split(X_flat, y), 1):
        print(f"\n{'=' * 60}")
        ...
        base_clf = create_base_model()

        # 내부에서 다시 CV 돌려서 확률 보정 (Platt scaling)
    ...
    print(f"\n{'=' * 60}")
    print("=== CV Summary (Calibrated LinearSVC) ===")
    print(f"{'=' * 60}")
    ...
    print("\n=== Train final calibrated model on ALL samples & save ===")
    base_clf = create_base_model()
    ...
    model_path = os.path.join(DATASET_DIR, "linearsvc_calibrated.pkl")
    joblib.dump(final_clf, model_path)
    print(f"✅ Final calibrated model saved to: {model_path}")
